...
export function AuthPage({ registerAccount = false }: { registerAccount?: boolean }) {
  ...
  const submit = handleSubmit(async values => {
    setError('');
    try {
      await api<void>(registerAccount ? '/identity/register' : '/identity/login', { method: 'POST', body: values });
      resetField('password');
  ...
  if (created) return <section className="auth-page"><h1>Konto erstellt</h1><p>Sie können sich jetzt anmelden.</p>
    <Button component={Link} to="/login" variant="contained">Zur Anmeldung</Button></section>;
