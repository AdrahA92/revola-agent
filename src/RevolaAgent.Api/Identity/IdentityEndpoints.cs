...
public static class IdentityEndpoints
{
    ...
    private static async Task<IResult> Register(Credentials request, IWebHostEnvironment environment,
        UserManager<ApplicationUser> users, RevolaDbContext db, IIdentityDelivery delivery, CancellationToken ct)
    {
        ...
        try
        {
            await delivery.SendAsync(new(user.Email!, user.Id, "confirm",
                IdentitySecurity.Encode(await users.GenerateEmailConfirmationTokenAsync(user))), ct);
        }
        catch (System.Net.Mail.SmtpException)
        {
            return Results.Problem(statusCode: 503, title: "Lokaler Test-Posteingang ist nicht erreichbar.");
        }
        ...
        SignInManager<ApplicationUser> signIn, RevolaDbContext db, CancellationToken ct)
    {
        ...
        {
            if (string.IsNullOrEmpty(request.Code) && string.IsNullOrEmpty(request.RecoveryCode))
                return Results.Json(new { requiresMfa = true }, statusCode: 202);
            if (!await IdentitySecurity.VerifySecondFactor(users, user, request.Code, request.RecoveryCode))
            {
                await users.AccessFailedAsync(user);
                return Results.Unauthorized();
            }
            if (!(await users.ResetAccessFailedCountAsync(user)).Succeeded) return Results.Conflict();
        }
        ...
        properties.Items["revola.session"] = session.Id.ToString();
    ...
    public sealed record Credentials(string Email, string Password, string? Code = null, string? RecoveryCode = null);
